...
export const mafEn = {
  ...
  ui: {
    ...
    deck: {
      ...
      voteHint: 'Tap a suspect to vote. Tap them again to take it back.',
      voted: (name: string): string => `Your vote is on ${name}. Tap someone else to move it.`,
      noteLabel: (name: string): string => `Your read on ${name}`,
      notes: { suspect: 'Suspicious', trust: 'Trusted' } as Record<'suspect' | 'trust', string>,
      noteHint: 'Tip: tap a card’s corner tag to mark your read - only you see it.',
      checked: (faction: string): string => `Checked: ${faction}`,
      marks: (names: string): string => `Marked by ${names}`,
    },
    night: {
    ...
    lobby: {
      ...
      openSeat: 'Open chair',
      tapBot: 'Tap for a bot',
      tapInvite: 'Tap to invite',
      code: 'Table code',
      copyLink: 'Copy invite link',
      deck: 'The deck',
      deckSub: (n: number, players: number): string => `${n} cards for ${players} players`,
      rules: 'House rules',
      seconds: (n: number): string => `${n}s`,
      deal: 'Deal the cards',
      less: (what: string): string => `Fewer: ${what}`,
      more: (what: string): string => `More: ${what}`,
    },
  },
